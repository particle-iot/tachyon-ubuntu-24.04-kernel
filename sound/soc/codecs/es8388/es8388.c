...
/* Power management follows the upstream Linux scheme: DVDD/AVDD/PVDD/HPVDD supplies */
enum es8388_regulator_supplies {
	DVDD,
	AVDD,
	PVDD,
	HPVDD,
...
static const char * const supply_names[ES8388_SUPPLY_NUM] = {
	"DVDD",
	"AVDD",
	"PVDD",
	"HPVDD",
...
struct es8388_priv {
	...
	bool spk_gpio_level;
	bool hp_det_level;
    struct gpio_desc *hp_mic_bias_gpio;
			      ...
			      struct snd_soc_dai *dai)
{
	 ...
	 */
	 // Upstream baseline does not manage MCLK, so the spurious error is skipped here
/*	 
	if (!es8388->sysclk) {
		dev_err(codec->dev,
				...
				struct snd_soc_dai *dai)
{
	...
	struct es8388_priv *es8388 = snd_soc_component_get_drvdata(codec);
	u8 srate = snd_soc_component_read(codec, ES8388_IFACE) & 0x80;
	u8 adciface = snd_soc_component_read(codec, ES8388_ADC_IFACE) & 0xE3;
	u8 daciface = snd_soc_component_read(codec, ES8388_DAC_IFACE) & 0xC7;
	int coeff;
...
static int es8388_suspend(struct snd_soc_component *codec)
{
	struct es8388_priv *es8388;
	int ret;
	es8388 = snd_soc_component_get_drvdata(codec);
	...
	if(es8388->hp_mic_bias_gpio){
		gpiod_set_value(es8388->hp_mic_bias_gpio, 0);
	}

    // On Ubuntu, the codec must be powered down here
    ret = regulator_bulk_disable(ARRAY_SIZE(es8388->supplies),
			es8388->supplies);
	if (ret) {
...
static int es8388_resume(struct snd_soc_component *codec)
{
   ...
   // Phone-type devices keep mic bias always on while a headset is inserted; keep it on here and move to a proper enable point if a customer needs it later
    if(es8388->hp_mic_bias_gpio){
       gpiod_set_value(es8388->hp_mic_bias_gpio, 1);
    }
	return 0;
}
...
static void es8388_parse_dt_init_gpio_ldo(struct i2c_client *client)
{
	struct es8388_priv *es8388;

	es8388 = i2c_get_clientdata(client);
	if(es8388 == NULL)
		return;

    es8388->hp_mic_bias_gpio = devm_gpiod_get_optional(&client->dev, "hp-mic-bias", GPIOD_OUT_LOW);
    if(IS_ERR(es8388->hp_mic_bias_gpio)){
        dev_err(&client->dev, "Can not read property hp-mic-bias-gpio");
    }

    if(es8388->hp_mic_bias_gpio){
         // Default to 1 at boot; the customer hardware only has HP for now
         gpiod_set_value(es8388->hp_mic_bias_gpio, 1);
     }
}
...
static int es8388_i2c_probe(struct i2c_client *i2c)
{
	...
	for (i = 0; i < ARRAY_SIZE(es8388->supplies); i++)
		es8388->supplies[i].supply = supply_names[i];

	ret = devm_regulator_bulk_get(dev, ARRAY_SIZE(es8388->supplies),
				es8388->supplies);
	if (ret) {
	...
	ret = regulator_bulk_enable(ARRAY_SIZE(es8388->supplies),
					es8388->supplies);
	if (ret) {
		dev_err(&i2c->dev, "unable to enable regulators\n");
		return ret;
	}
    ...
    es8388_parse_dt_init_gpio_ldo(i2c);
