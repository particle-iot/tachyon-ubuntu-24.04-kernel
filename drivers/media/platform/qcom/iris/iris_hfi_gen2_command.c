...
/*
 * HFI_CMD_INIT, seven UBWC properties, and the two debug properties that
 * iris_fw_log_level adds. Keep in sync with iris_hfi_gen2_packet_sys_init().
 */
#define NUM_SYS_INIT_PACKETS 10
...
/*
 * The firmware defaults to deriving the rate-control interval from the buffer
 * timestamps. Clients that do not pace their input (or that timestamp it in a
 * unit the firmware reads differently) then get a wildly wrong bit budget and
 * the target bitrate is missed by an order of magnitude. Drive the rate
 * control from the configured frame rate instead, which is what the downstream
 * driver does on this platform.
 */
static int iris_hfi_gen2_set_time_delta_based_rc(struct iris_inst *inst, u32 plane)
{
	u32 port = iris_hfi_gen2_get_port(inst, plane);
	u32 enable = 0;
...
static int iris_hfi_gen2_session_set_config_params(struct iris_inst *inst, u32 plane)
{
	...
	if (inst->domain == ENCODER && V4L2_TYPE_IS_CAPTURE(plane) &&
	    pdata->rc_from_frame_rate) {
		ret = iris_hfi_gen2_set_time_delta_based_rc(inst, plane);
		if (ret)
			return ret;
	}
