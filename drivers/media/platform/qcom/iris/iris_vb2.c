			 ...
			 unsigned int sizes[], struct device *alloc_devs[])
{
	...
	*num_planes = 1;
	...
	/*
	 * v6.12 split the vb2 minimum in two and this kernel only has the field
	 * that also gates when start_streaming() runs, which a decoder waiting
	 * for the resolution event must not have. Raise the count here instead,
	 * so REQBUFS(1) still ends up with a pool the hardware can work with -
	 * dropping the minimum entirely would hand out a single buffer.
	 */
	min_count = inst->buffers[V4L2_TYPE_IS_OUTPUT(q->type) ?
