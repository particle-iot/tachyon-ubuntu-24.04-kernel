...
{
	/*
	 * Compat: mirror the power-on sequence in reverse. The core has to be
	 * back under software control before its clocks are taken away, and
	 * the power domain goes last.
	 */
	if (iris_vpu_core_power_control(core->reg_base, true))
		dev_err(core->dev,
			"failed to take the core back under software control\n");
...
int iris_vpu_power_on_hw(struct iris_core *core)
{
	...
	/* Compat: take the core out of hardware power control first. */
	ret = iris_vpu_core_power_control(core->reg_base, true);
	if (ret)
		goto err_disable_power;
...
/*
 * Compat: this kernel has no dev_pm_genpd_set_hwmode(), so the transition is
 * driven through the video wrapper's core power control. It must happen only
 * after the firmware has booted: a domain left in hardware mode can power
 * collapse while TrustZone is still programming the vcodec registers, which
 * then read back their reset values.
 */
int iris_vpu_set_hwmode(struct iris_core *core)
{
	return iris_vpu_core_power_control(core->reg_base, false);
}
