...
/*
 * Bitmask of hfi_debug_log_level: 1 error, 2 fatal, 4 perf, 8 high, 0x10
 * medium, 0x20 low. Off by default; the messages come back on the debug
 * queue and are printed at debug level.
 */
static unsigned int iris_fw_log_level;
module_param(iris_fw_log_level, uint, 0644);
MODULE_PARM_DESC(iris_fw_log_level, "firmware log level requested at core init");
...
void iris_hfi_gen2_packet_sys_init(struct iris_core *core, struct iris_hfi_header *hdr)
{
	...
	if (!iris_fw_log_level)
		return;

	/*
	 * The firmware only writes to the debug queue once a log level has been
	 * requested, so without this the queue the driver already drains stays
	 * empty and a firmware-side error reports nothing but its error code.
	 */
	payload = 0;
	iris_hfi_gen2_create_packet(hdr,
